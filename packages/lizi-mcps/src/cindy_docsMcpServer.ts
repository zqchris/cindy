...
/**
 * 正式文档(PPT / Word / PDF)动手前的固定工序:先写 HTML 设计稿定版式,再据此
 * 还原成目标格式。
 *
 * 为什么值得多花一步:模型排 HTML 比直接排 pptx/docx 结构准得多 —— 在 HTML 里
 * 能一眼看出层级、留白、对齐是否成立,而直接填 slides 数组只能凭想象。先定版式
 * 再还原,成品比直接生成好看一大截(这正是「纯白只有字」的解法)。
 *
 * 挂在**工具描述**上而不是只写进某类会话的提示词:模型选工具时一定会读到它,
 * 而且伙伴会话、普通会话、三种 harness 全都覆盖,不依赖任何 skill 是否安装。
 *
 * 表格不走这一步 —— xlsx 的美观度取决于列宽、表头、数字格式,HTML 帮不上忙。
 */
const D_DESIGN_CORE =
...
const D_DESIGN_VIA_HTML =
  D_DESIGN_CORE +
  '做法是先在 `tmp/` 写一份自包含 HTML 设计稿(样式内联)把版式落定,再对着它逐块还原成' +
  '本工具的结构化参数。**这是你自己的工序,用户看不到也不该看到** —— 不要把设计稿发给他、' +
  '不要问他「要不要先看草稿」、不要把 `tmp/` 路径念给他听,他只收 `documents/` 里的成品。' +
